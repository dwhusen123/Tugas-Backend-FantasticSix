...
const addNotes = async(req, res) => {
    ...
    try {
        const data = await quey("INSERT INTO notes (title, datetime, note) VALUES (?,?,?,)" , [title, datetime, note]);
...
const deleteNotes = async(req, res) => {
    ...
    try {
        ...
        return res.status(200).json({msg:"berhasil dhapus"})
    } catch (error) {
        return res.status(400).json({msg:"gagal dihapus"})
    }
}

export {getNotes, addNotes, getNotesId, updateNotes, deleteNotes};